...
public class ChronoGraphQueryProcessor {
    ...
    public Iterator<Vertex> getVerticesBySearchSpecifications(final Set<SearchSpecification<?,?>> searchSpecifications) {
        ...
        Set<String> indexedProperties = Sets.newHashSet(indexManager.getIndexedVertexPropertyNames());
        Set<ChronoGraphIndex> dirtyIndices = indexManager.getDirtyIndices();
        indexedProperties.removeAll(
            dirtyIndices.stream()
                .filter(i -> Vertex.class.equals(i.getIndexedElementClass()))
        ...
        if (indexedProperties.containsAll(propertyToSearchSpecifications.keySet())) {
            ...
            if (indexedPropertiesToUse.isEmpty()) {
                ...
                ChronoLogger.logWarning(
                    "Query requires iteration over all vertices, because none of the given properties is indexed (or the indices are dirty)!"
    ...
    public Iterator<Edge> getEdgesBySearchSpecifications(final Set<SearchSpecification<?,?>> searchSpecifications) {
        ...
        indexedProperties.removeAll(
            dirtyIndices.stream()
                .filter(i -> Edge.class.equals(i.getIndexedElementClass()))
                .map(ChronoGraphIndex::getIndexedProperty)
                .collect(Collectors.toSet())
        );
    ...
    private Iterator<Vertex> performVertexQueryOnIndex(final Set<SearchSpecification<?,?>> searchSpecs) {
        ...
        ChronoGraphIndexManagerInternal indexManager = this.getIndexManager();
        Iterator<String> indexQueryResultIdIterator = indexManager.findVertexIdsByIndexedProperties(this.tx, searchSpecs);
        // we now enhance this iterator by looking at the modifications performed by the user in the transaction context
        GraphTransactionContextInternal context = (GraphTransactionContextInternal) this.tx.getContext();
        if (context.isDirty()) {
            // query context is dirty and requires post-processing to properly reflect the transient state
            List<Property<?>> transientMatches = Lists.newArrayList();
            for (SearchSpecification<?,?> searchSpec : searchSpecs) {
                transientMatches.addAll(context.getModifiedVertexProperties(searchSpec));
            }
            ...
            while (indexQueryResultIdIterator.hasNext()) {
                ...
                if (context.isVertexModified(id)) {
                    // The vertex itself may have been modified, but the concrete property/properties we are
                    // looking for may still be unmodified.
                    Vertex modifiedVertex = context.getModifiedVertex(id);
                    if (filterPredicate.apply(context.getOrCreateVertexProxy(modifiedVertex))) {
    ...
    private Iterator<Edge> performEdgeQueryOnIndex(final Set<SearchSpecification<?,?>> searchSpecs) {
        ChronoGraphIndexManagerInternal indexManager = this.getIndexManager();
        Iterator<String> indexQueryResultIdIterator = indexManager.findEdgeIdsByIndexedProperties(this.tx, searchSpecs);
        // we now enhance this iterator by looking at the modifications performed by the user in the transaction context
        GraphTransactionContextInternal context = (GraphTransactionContextInternal) this.tx.getContext();
        if (context.isDirty()) {
            // query context is dirty and requires post-processing to properly reflect the transient state
            List<Property<?>> transientMatches = Lists.newArrayList();
            for (SearchSpecification<?,?> searchSpec : searchSpecs) {
                transientMatches.addAll(context.getModifiedEdgeProperties(searchSpec));
            }
            ...
            while (indexQueryResultIdIterator.hasNext()) {
                ...
                if (context.isEdgeModified(edgeId)) {
                    // The edge itself may have been modified, but the concrete property/properties we are
                    // looking for may still be unmodified.
                    Edge edge = context.getModifiedEdge(edgeId);
                    if (filterPredicate.apply(context.getOrCreateEdgeProxy(edge))) {
            ...
            return new EdgeResolvingIterator(resultSet.iterator(), ElementLoadMode.LAZY);
        } else {
            // query context is clean, no modifications, so index query delivers the result immediately
            return new EdgeResolvingIterator(indexQueryResultIdIterator, ElementLoadMode.LAZY);
        }
    }
