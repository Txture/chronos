...
public interface ChronoGraphIndexManager {
	 ...
	 * 			  The metadata for the commit of dropping an index. May be <code>null</code>.
	 */
	public void dropIndex(ChronoGraphIndex index, Object commitMetadata);
	 ...
	 * This operation cannot be undone. Use with care.
	 * @param commitMetadata
	 * 			  The metadata for the commit of dropping all indices. May be <code>null</code>.
	 */
	...
	/**
	 * Returns the indices which are known and currently {@linkplain #getDirtyIndices() clean}.
	 *
	 * @return The set of all indices, without the ones which are currently dirty. May be empty.
	 */
	public default Set<ChronoGraphIndex> getCleanIndices(){
		Set<ChronoGraphIndex> indices = Sets.newHashSet(this.getAllIndices());
		indices.removeAll(this.getDirtyIndices());
		return indices;
	}
