...
public interface IndexManager {
	 ...
	 *
	 * <p>
	 * This method acts as an "umbrella" method for its other overloads, e.g.
	 * {@link #addIndexer(String, StringIndexer)}. Please note that this method will redirect to those other methods,
	 * and throw a {@link ChronoDBIndexingException} if no suitable overload was found.
	 *
	 * <p>
	...
	public default void addIndexer(final String indexName, final Indexer<?> indexer) {
		...
		// no method matched...
		throw new ChronoDBIndexingException(
				"Failed to add indexer. Did you implement the Indexer interface directly? Supported sub-interfaces are: "
						+ supportedIndexerTypes);
	}
	 ...
	 * <p>
	 * Please note that for any index, there must be <b>at most one</b> indexer that accepts any given object. If there
	 * are multiple indexers on the same index that accept a given object upon insertion, an
	 * {@link IndexerConflictException} is thrown when the object is inserted.
	 *
	 * <p>
	 * If multiple indexers are registered to the same index name, then all indexers must have the same type (i.e. you
	 * cannot mix {@linkplain StringIndexer string} and {@linkplain LongIndexer long} indexers in the same index).
	 *
	 * <p>
	 * Adding an indexer dirties the index. Consider calling {@link #reindexAll()} when all changes to the indexers are
	 * complete.
	 ...
	 *
	 * @param force
	 *            Use <code>true</code> to force a re-build of all indices.
	 */
	 ...
	 *         <code>null</code>.
	 */
	public Set<String> queryIndex(final long timestamp, Branch branch, String keyspace,
