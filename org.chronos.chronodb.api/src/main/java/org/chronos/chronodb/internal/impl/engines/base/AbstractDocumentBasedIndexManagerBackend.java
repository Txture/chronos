...
public abstract class AbstractDocumentBasedIndexManagerBackend implements DocumentBasedIndexManagerBackend {
	...
	protected AbstractDocumentBasedIndexManagerBackend(final ChronoDB owningDB) {
	 ...
	 * @param keys
	 *            The set of keys to roll back. If the set is <code>null</code>, <b>all</b> keys will be rolled back. If the set is empty, no keys will be rolled back. If the set is non-empty, only the given keys will be rolled back.
	 */
	protected void rollbackInternal(final long timestamp, final Set<String> branches, final Set<QualifiedKey> keys) {
		...
		Set<ChronoIndexDocument> documents = this.getDocumentsTouchedAtOrAfterTimestamp(timestamp, branches);
	 ...
	 * @param branches
	 *            The set of branches to retrieve the documents for. If <code>null</code>, documents from all branches will be retrieved. If this set is empty, then the result set will also be empty.
	 *
